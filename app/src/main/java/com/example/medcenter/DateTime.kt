...
class DateTime : AppCompatActivity() { 
    ...
    private fun saveAvailableTimes(date: String) { 
        val availableTimes = (9..19).map { hour -> String.format("%02d:00", hour) // Генерация времени по типу 9:00, 10:00 и т.д.
        } val appointmentData = hashMapOf( "date" to date, "availableTimes" to availableTimes ) 
        db.collection("appointments").add(appointmentData)
            .addOnSuccessListener { documentReference -> 
            Log.d("Firestore", "Available times added with ID: ${documentReference.id}") }
            .addOnFailureListener { e -> Log.w("Firestore", "Error adding available times", e) } } 
    ...
    { db.collection("appointments") .whereEqualTo("date", date) .get()
            .addOnSuccessListener { result -> for (document in result) 
                { val availableTimes = document.get("availableTimes") as List<String> populateTimeButtons(availableTimes, cardView) } }
            .addOnFailureListener { exception -> Log.e("Firestore", "Error getting available times: ", exception) } } 
    private fun populateTimeButtons(availableTimes: List<String>, cardView: View) {
        ...
        availableTimes.forEach { time -> val button = Button(this) 
            button.text = time 
            ...
            button.setOnClickListener { 
                ...
                Toast.makeText(this, "Выбрано время: $time", Toast.LENGTH_SHORT).show() // Здесь можно сохранить выбранное время в Firestore или выполнить другие действия 
